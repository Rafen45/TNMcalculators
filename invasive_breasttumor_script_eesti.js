...
$(document).ready(function(){
    ...
    $("input[name='tumor_margins_pos_cbox']:not('#tumor_margins_pos_cbox_CD')").on("click", function(){
        cboxInput(this, " ", "Täpsusta ulatus")
    })
    $("#tumor_margins_pos_cbox_CD").on("click", function(){
        cboxInput(this, null, "Täpsusta")
        disable_class(this)
    })
    ...
    $("#tumor_site_Clock_position").on("click", function(){
        cboxInput(this, "only_numbers addunits")
    })
    ...
    $('#regional_lymph_nodes').change(function () {
        if ($('#regional_lymph_nodes').val() === "Sisaldavad kasvajarakke") {
            $('#regional_lymph_nodes_block_general').slideUp("slow");
            $('#regional_lymph_nodes_block_specified').slideDown("slow");
        } else if ($('#regional_lymph_nodes').val() === "Ei sisalda kasvajarakke") {
...
const archPatternObj = {
    ...
    heading: "</br>Arhitektuurilised mustrid: ",
...
const tumExtObj = {
    ...
    heading: "</br>Tuumori ulatus: </br> &#8212; ",
    join: "</br> &#8212; "
}
const tumMarPosObj = {
    input_div_id: "#tumor_margin_pos_cbox_div",
    heading: "Positiivne invasiivse kasvaja osas: ",
    join:  " ",
    extra_text: "&#8195; Täpsustus: "
}
const tumMarPosOtherObj = {
    input_div_id: "#tumor_margins_positive_other_div",
    heading: "Kaugus teistest piiridest: </br> &#8212;",
    join:  "</br> &#8212; ",
}
...
$('#calc_results_btn').on("click", function () {
    $('#results_block').empty();
    addCheckbox("tumor_site_chkbx", "Tuumori asukoht: ")
    HeadingObjArray.forEach(element => addGenericInput(element))
    $('#results_block').append(sumtable("#lymph_nodes_tables", "<br/>Lümfisõlmed: ", "<br/>Kokku uuritud lümfisõlmi: "))
    HeadingObjArray2.forEach(element => addGenericInput(element))
    $("#results_block").append("</br>" + "TMN klassifikatsioon: " + calcGradeT() + calcGradeN() + calcGradeM() + "<br/>")
    $('#results_title').show();
    $('#results_block').show();
    $('#copy_result').text('');
